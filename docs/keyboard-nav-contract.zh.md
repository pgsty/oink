...
前置阅读：`docs/navigation-contract.md`、`CLAUDE.md`

本文允许讨论与取舍；评审后公共决策冻结进 contract
...
1. **可配置，默认开启**。`params.ui.keyboard_nav`，主题默认 `true`。
...
- `action-registry.js` 暴露 `global.OinkActions`，内建 `copy_markdown`
  动作与剪贴板降级；manifest 经 `#td-action-manifest` JSON 注入。
...
- 侧栏树 DOM：`nav.td-shell-tree` → `li.td-shell-tree__item`
  （`td-has-child` / `td-leaf` / `--hidden`）→ `div.td-shell-tree__row` →
  `a.td-shell-tree__link` + 可选 chevron 按钮 →
  `div.td-shell-tree__children`（`td-is-open` / `--static`）。
- docs 页当前**没有** pager（阅读契约才引入卡片式树序 pager，
...
> `.td-shell-tree__children` 既非 `td-is-open` 也非 `--static`。
...
（仅 `$localSearch` 装配时存在），且不受 `ui.keyboard_nav`
...
该段落 + 同步 `scripts/check-palette.py`（如其断言涉及 seed）+
`tests/js/command-palette.test.js`、`palette-model.test.js`
相应用例。机器契约 `tests/fixtures/navigation/contract.json` 的
...
零对话框逻辑重复。面板未装配（`offline_search` 关闭）时 f/c 静默且
...
- `tests/js/keyboard-nav.test.js`（仿 prd4 系测试）：
  守卫谓词（IME / 输入焦点 / 修饰键 / 弹层开启）、扁平化过滤、
  `a/d` 状态机（展开→折叠→父项）、`h/l` 三级数据源解析、
  边界驻留（顶/底不回绕）。
- `tests/js/command-palette.test.js`：`/` 搜索态、`\` 命令态
  用例更新。
- 新增 `scripts/check-keyboard.py`：断言 scripts.html 门控与
...
| 3   | `ui.keyboard_nav: true` 默认值                                     | 主题 `hugo.yaml`                                               |
| 4   | `/` `\` 变更 + 导航契约修订 + check/test 同步（同一原子变更）    | 主题 `command-palette.js` / `docs/` / `scripts/` / `tests/js/` |
| 5   | `.td-kbd-focus` 焦点高亮 + 禅模式样式                              | 主题 `assets/scss/`                                            |
| 6   | `keyboard-nav.test.js` + `check-keyboard.py`             | 主题 `tests/js/` / `scripts/`                                  |
...
里程碑归属：本特性无外部依赖，可独立发布；与阅读契约的树序 pager
