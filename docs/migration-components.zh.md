...
展示能力更新：OINK 0.4.1（`reading_width`、`example`、`contributors` 与
...
`book-examples`，四种编号组件都获得原生形态（见 `docs/book-contract.md` 版本 2）
...
[阅读/发布契约](reading-release-contract.md)、
[Landing 契约](landing-contract.md)与
[Book 契约](book-contract.md)。
...
`docs_sidebar_root` 只接受默认值 `section` 或 `home`；侧栏可见树与 Pager
顺序共同使用同一个解析结果。
...
- 删除复制的 `robots.txt`、404、lastmod、td-section-index、search-metadata 或
...
outputs:
  ...
  section: [HTML, print]
params:
  # slim | normal | wide；Book 默认为 normal。
...
`reading_width` 只控制 Book 正文内层宽度：`slim` 是紧凑的文字栏，`normal`
让正文与普通图片、代码块对齐，`wide` 则铺满文章画布。它与控制外层框架的
`page_width` 相互独立，也可在页面 front matter 中用同名键单独覆盖。
...
遇到下表未覆盖的遗留写法时，一次性站点脚本应当：

1. 盘点每个遗留目标及其附着的图注文本；
2. 从现有文本提出编号，但不修改文件路径；
3. 把伪装成“引用”的正文链接改为 `xref`；
4. 图注缺失或含糊时停止，不猜测；
...
OINK 提供一个默认 dry-run 的可执行工具 `scripts/migrations/book_figures.py`。
其 profile 以推导来源的语料命名，每个只识别一类 Book 之前的遗留写法，
无法确定的一律报告而不猜测：

| Profile | 识别的遗留写法 |
| --- | --- |
| `tpme` | 图片上方的 h6 伪图注、编号表格，以及跨语言 `/en/…#anchor` fragment 泄漏 |
| `ddia-v2` | 一个把图、编号表格与代码示例混在一起的站点自有 `figure` shortcode |
| `ddia-v1` | 裸图加其后的图注段落，以及充当引用的图片链接 |
| `pg-internal` | 相邻的图注/图片对、编号表格，以及不依赖目标先后次序的正文编号引用 |

工具默认只输出报告，`--write` 必须显式给出，且对同一棵树第二次运行必须零改动。
其安全契约由双 Hugo 矩阵中的 `scripts/check-book-migrations.py` 覆盖。
