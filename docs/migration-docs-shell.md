# Docs shell and page-end migration guide
...
[简体中文](migration-docs-shell.zh.md)
...
interactive search check with the documented environment override (`x` is
Hugo's alternate key delimiter; `_` cannot be used because the key contains
underscores):
