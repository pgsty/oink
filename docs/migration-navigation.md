# Navigation migration and configuration reference
...
[machine-checked contract](navigation-contract.md).
...
3. publish a tagged theme release naming the navigation feature set;
...
Navigation adds behavior, not another information architecture.
...
Flat menus need no migration. To adopt the grouped menus deliberately:
...
Local search stays offline-capable and language-separated. The Palette extends the
...
`params.offline_search_index` controls optional text fields:
...
`params.offline_search_max_results` limits page hits; matching Actions are grouped
...
The corresponding page and Palette actions share descriptors and URL
...
activation time. Other built-in URL actions keep an href that matches the shared
...
earlier TOC-rail list. Historical rail-only actions outside the built-in
...
1. `params.offline_search` is true;
...
- [ ] Changelog and tag name the navigation feature set.
