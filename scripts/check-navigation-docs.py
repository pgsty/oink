#!/usr/bin/env python3
"""Verify EN/ZH reference parity, release language, and starter builds."""
...
EN_DOC = ROOT / "docs" / "migration-navigation.md"
ZH_DOC = ROOT / "docs" / "migration-navigation.zh.md"
...
def build_example(base_url: str, destination: Path) -> None:
    ...
    require(
        'data-td-sidebar-icon-policy="all"' in docs,
    ...
    require(
        'id="td-shell-search"' in docs and 'id="td-action-manifest"' in docs,
...
def main() -> int:
    try:
        ...
        require(
            "migration-navigation.md" in readme
            and "migration-navigation.zh.md" in readme,
        ...
        with tempfile.TemporaryDirectory(prefix="oink-navigation-docs-") as temporary:
        ...
        print(f"navigation documentation check failed: {exc}", file=sys.stderr)
        return 1

    print("bilingual navigation migration and starter checks passed")
    return 0
