#!/usr/bin/env python3
"""Validate the dry-run-first PRD 5 Book migration profiles."""
...
MIGRATOR = ROOT / "scripts/migrations/prd5_book_migrate.py"
...
def main() -> int:
    try:
        require(MIGRATOR.exists(), "PRD 5 migration tool is missing")
        with tempfile.TemporaryDirectory(prefix="oink-prd5-migrations-") as temp:
            base = Path(temp)
            cases = (
                (
                    "tpme",
        ...
        print(f"PRD 5 migration checks failed: {exc}", file=sys.stderr)
        return 1
    print("PRD 5 Book migration checks passed")
    return 0
