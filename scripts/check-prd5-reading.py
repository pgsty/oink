#!/usr/bin/env python3
"""Validate PRD 5 reading primitives: math passthrough and pager behavior."""
...
def check_explicit_navigation(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="oink-prd5-explicit-nav-") as temp:
        source = Path(temp)
...
def check_home_root_navigation(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="oink-prd5-home-root-") as temp:
        source = Path(temp)
...
def check_invalid_pager_config(hugo: str) -> list[str]:
    errors: list[str] = []
    cases = (
        ("bad-type", "", "    pager:\n      types: [docs, archive]", "invalid params.ui.pager.types value"),
        ("scalar-types", "", "    pager:\n      types: docs", "params.ui.pager.types must be an array"),
        ("scalar-pager", "", "    pager: docs", "params.ui.pager must be a map"),
        ("page-string", 'pager: "false"\n', None, "front matter pager must be a boolean"),
        ("bad-docs-root", "", "    docs_root: archive\n    pager:\n      types: [docs, book, blog]", "params.ui.docs_root must be home or section"),
        ("scalar-docs-root", "", "    docs_root: true\n    pager:\n      types: [docs, book, blog]", "params.ui.docs_root must be a string"),
    )
    for name, front_matter, config_extra, expected in cases:
        with tempfile.TemporaryDirectory(prefix=f"oink-prd5-pager-{name}-") as temp:
            source = Path(temp)
            create_theme_site(source)
...
def check_rss_pager_output(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="oink-prd5-pager-rss-") as temp:
        source = Path(temp)
        create_theme_site(source)
        config_path = source / "hugo.yaml"
        config = config_path.read_text()
...
def main() -> int:
    ...
    if args.public is not None:
        ...
        with tempfile.TemporaryDirectory(prefix="oink-prd5-reading-") as temp:
            public = Path(temp) / "public"
            result = build_example(args.hugo, public)
            if result.returncode != 0:
                print("PRD 5 reading fixture failed to build:")
    ...
    if errors:
        print("PRD 5 reading checks failed:")
        for error in errors:
            print(f"  {error}")
        return 1
    print("PRD 5 reading checks passed")
    return 0
